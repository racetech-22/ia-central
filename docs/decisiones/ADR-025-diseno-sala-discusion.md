...
## Enmienda 2026-08-07: por qué §9 no queda redundante aunque ACP estabilice su transporte remoto

Verificado el 2026-08-07 contra el RFD "Streamable HTTP & WebSocket Transport" (`https://agentclientprotocol.com/rfds/streamable-http-websocket-transport`, ver también enmienda de esta fecha a ADR-027 punto b): en ese transporte **el Cliente se conecta hacia el Agente/Servidor** — la propia especificación establece que *"Clients that support remote ACP over HTTP MUST support both Streamable HTTP and WebSocket"*, con el cliente como iniciador de la conexión hacia el agente remoto.

Bajo el modelo de ADR-027 (IA CENTRAL es el cliente ACP, el Ejecutor es el agente), eso significa que si un proyecto externo corriera en la PC local de Fernando, **IA CENTRAL tendría que conectarse hacia adentro de esa PC** — exactamente la dirección que este punto 9 descartó desde el origen, por no exponer la PC local a conexiones entrantes (ver "Alternativas descartadas": *"Conexión entrante hacia la PC local de Fernando... descartado — expondría la PC local a conexiones entrantes"*). Que ACP estandarice un transporte remoto no cambia esa dirección: sigue siendo cliente-hacia-agente, no agente-hacia-cliente. El protocolo saliente de este punto 9 (destino inicia la conexión hacia IA CENTRAL, nunca al revés) sigue siendo la única vía compatible con ese requisito, sin importar cuánto madure el transporte remoto de ACP.

## Abierto, no resuelto

- Pantalla exacta del panel para "conectar proyecto nuevo" — no diseñada, ahora además debe cubrir instalación/autenticación de cualquier agente del catálogo de ADR-027, y aislamiento de proceso (ADR-028) para destinos externos, no resuelto por esa ADR (ver su punto 5).
...
- Diseño concreto del aislamiento de proceso por Ejecutor — resuelto en ADR-028 (2026-08-07) para el caso nativo; queda abierto el caso de destinos externos (ver ADR-028 y el punto anterior de esta lista). Nada de ADR-028 está implementado todavía.
