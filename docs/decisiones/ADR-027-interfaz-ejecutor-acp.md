...
- **Si `session/request_permission` conserva el corte determinista del hook de ADR-007, o si un agente puede ejecutar herramientas sin pasar por los métodos de archivo y terminal del cliente — sin verificar.** Es la pregunta que más pesa: si no se sostiene, se caen el modelo de permisos, la exclusión de secretos y el catálogo entero.
