...
- **Falso positivo confirmado en la práctica**: el script matchea sobre el string completo del comando, no sobre su intención. Al escribir el commit que agregó este mismo hook, dos mensajes de `git commit -m "..."` fueron bloqueados solo por *mencionar como texto descriptivo* la sentencia SQL de borrado y la frase sobre borrar archivos en la carpeta de backups — sin ejecutar nada destructivo. Al documentar o commitear cambios relacionados con este hook, hay que evitar escribir esas frases gatillo de forma literal en el propio comando (por ejemplo, describir el hook en términos más indirectos, como se hizo en el commit que finalmente pasó).
