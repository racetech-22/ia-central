...
- `${ORCHESTRATOR_CREDENTIALS_PATH:-/home/fernando/.claude/.credentials.json}:/home/orchestrator/.claude/.credentials.json:ro` — mismo archivo, mismo patrón de solo lectura que ADR-017 ya probó en un contenedor descartable. Verificado que el SDK no reimplementa autenticación: lanza el mismo binario `claude` real como subproceso y hereda el entorno del proceso Python (confirmado leyendo `_internal/transport/subprocess_cli.py` del SDK) — la autenticación es idéntica a la de la CLI directa. Corregido en revisión (Fernando/Cowork, antes del merge): la ruta origen del mount usaba `${HOME}` del usuario que corre `docker compose`, no una variable propia del proyecto — dependía de quién ejecutara el comando, no del dueño real del credencial. Pasó a `ORCHESTRATOR_CREDENTIALS_PATH`, documentada en `env.example` junto a `ORCHESTRATOR_UID`, mismo patrón exacto.
...
## Enmienda 2026-08-03 (misma fecha): las 4 tools residuales — investigadas, `DesignSync` tiene efecto real, no inerte

Investigación acotada de las cuatro tools que quedaron documentadas arriba como "no da acceso a filesystem ni shell" — esa afirmación era una suposición, no verificada en su momento. Verificado ahora contra la documentación oficial de Claude Code (`code.claude.com/docs/en/tools-reference`, `code.claude.com/docs/en/whats-new`) y confirmado con evidencia externa (fuera del código fuente de `claude-agent-sdk-python`, donde ninguna de las cuatro existe — son del binario `claude`, no de la librería del SDK):

- **`DesignSync`**: **tiene efecto real, no es inerte.** Lee y escribe proyectos de diseño en `claude.ai/design` a través del login de la cuenta. Tiene métodos de escritura real (`write_files`, hasta 256 archivos por llamada; `delete_files`) condicionados a un `planId` de un `finalize_plan` previo, y sus rutas de origen (`localPath`) deben estar "dentro del `localDir` del plan" — es decir, **puede leer archivos del filesystem local (el repo completo montado en `/app`) y subirlos a un proyecto externo en claude.ai**, sin pasar por `security.py` ni por ninguna de las dos tools MCP de este proyecto. Esto es exactamente el tipo de capacidad que ADR-017 condiciona a que "ninguna tool permita lectura de rutas arbitrarias" — `DesignSync` no es una tool MCP de este proyecto, pero rompe el espíritu de esa condición y de ADR-015 ("el LLM nunca tiene shell arbitrario... la lista de tools ES la política de seguridad") igual.
- **`Monitor`**: corre comandos en background y devuelve su salida a la conversación; la documentación oficial dice explícitamente que "usa las mismas reglas de permiso que Bash". No se probó en la práctica si de verdad puede ejecutar algo con `Bash` fuera de la lista de tools — dado que `DesignSync` ya alcanza para el hallazgo bloqueante, no hizo falta agotar esta vía para la conclusión de esta enmienda, pero **sigue sin confirmarse que sea inerte**.
- **`PushNotification`**: manda una notificación de escritorio y push al teléfono vía infraestructura de Anthropic — saca texto arbitrario del contenedor hacia un canal externo (aunque acotado a texto corto de notificación, no a volcado de archivos).
- **`RemoteTrigger`**: crea/actualiza/corre/lista *Routines* en claude.ai (lo que respalda `/schedule`) — el modelo podría programar disparadores automáticos nuevos en la nube, algo que contradice directamente el principio de esta ADR ("sin disparador automático").

**Test de remoción, verificado dos veces limpio**: `ClaudeAgentOptions(tools=[], disallowed_tools=["DesignSync", "Monitor", "PushNotification", "RemoteTrigger"])` sí las saca del `SystemMessage` (`'tools': []`, confirmado con una consulta real que además devolvió `PONG` sin error). `disallowed_tools` funciona como mecanismo de remoción donde `tools=[]` solo no alcanzaba.
