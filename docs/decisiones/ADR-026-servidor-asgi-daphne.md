...
- Implementado (2026-08-04): `daphne==4.2.3` en `requirements.txt`, `core/asgi.py` con `ProtocolTypeRouter` (rama `"http"`; sin rama `"websocket"` todavía, ver ADR-025 §9.7), y el servicio `web` de `docker-compose.yml` corriendo `daphne` en vez de `runserver`.
- Implementado: alta de `daphne` en `docs/DEPENDENCIAS.md` (ADR-019), en el mismo commit en que se agregó a `requirements.txt`.
- Verificado después de la migración que el admin de Django y `https://aicentral.network/admin/login/` siguen respondiendo como antes (`200`), con el mismo criterio de verificación end-to-end de ADR-021/ADR-022/ADR-023 — cierra el riesgo operativo que ADR-025 §9.9 había anotado.
- Cerró el pendiente "servidor ASGI a elegir" de ADR-025 §9.7, que ya apunta a esta ADR.
- **Pendiente (Fase 3)**: una vez que existan rutas de websocket (ADR-025 §9.7, subtarea siguiente), los límites de tamaño de mensaje y de frame de WebSocket de Daphne quedan en su default de 1 MiB (configurables por CLI, `0` los desactiva) — restricción de diseño real para las tramas de ADR-025 §9.3. Hoy no bloquea nada porque todavía no hay rutas de websocket y lo durable viaja por POST HTTP fuera del socket (ADR-025 §9.2), pero hay que tenerlo presente si alguna trama (`stream`, `permission_request`, etc.) creciera por encima de ese límite.
