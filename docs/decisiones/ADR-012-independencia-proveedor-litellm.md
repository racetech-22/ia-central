...
- **Enmienda (2026-08-01, ver ADR-016):** el punto 1 de la Decisión ("LiteLLM se monta como gateway desde el inicio de Fase 3") se implementó, pero el gateway no está en el camino de las llamadas a Claude mientras se use autenticación por suscripción en vez de clave de API. Sigue montado y disponible; el resto de la decisión (frontera `orchestrator.run(...)`, framework reemplazable) no se ve afectado.
