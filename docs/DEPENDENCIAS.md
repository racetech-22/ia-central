...
| claude-agent-sdk | `==0.2.89` (pin exacto — no la última, `0.2.128` al momento de fijarlo) | `orchestrator/requirements.txt` | ADR-012, ADR-017, ADR-021 | Manual, pinear versión exacta (nunca `latest`) | Servicio `orchestrator` de Docker Compose (`orchestrator/orchestrator.py`) |
