...
- ADR-025: Diseño técnico inicial de la sala de discusión (pilar 4 de ADR-024) — **en progreso**, no una decisión cerrada. Puente de permisos vía `can_use_tool`, roles como agente principal + sub-agente en una sesión del SDK (proyectos nativos), agente propio por destino con conexión saliente (proyectos externos, el SDK no tiene transporte remoto nativo). Autenticación por proyecto remoto y protocolo de conexión saliente quedan explícitamente sin resolver.
