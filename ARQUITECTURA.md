...
**Pendiente (Fase 3):** el agente que decida qué hacer y a qué IA/herramienta delegar cada tarea correrá como servicio de `docker-compose.yml` (`restart: always`), junto a `web` y `db` — no como servicio systemd ni sesión de tmux, para no salirse del contrato de portabilidad de ADR-002. Ver ADR-015. Se implementará con el Claude Agent SDK como motor del bucle, expuesto al resto del sistema (Django, MCP servers) únicamente vía una interfaz interna propia (`orchestrator.run(...)`) — el SDK queda pensado como detalle de implementación reemplazable, para no llamarse nunca directamente desde fuera de esa interfaz. Se enrutará a través de un gateway LiteLLM autohospedado, no directo a la API de Anthropic (ver ADR-012) — aunque mientras Fase 3 use autenticación por suscripción en vez de clave de API, el gateway queda montado y sano sin estar en el camino real de esas llamadas. Ver ADR-016. Hoy no existe ningún servicio de orquestador en `docker-compose.yml` (los 4 servicios reales son `db`, `web`, `litellm`, `ntfy`), ni código de orquestador en el repo, ni el Claude Agent SDK como dependencia declarada en `requirements.txt`.

**Pendiente (Fase 3):** el LLM que corra en el orquestador nunca tendrá shell arbitrario. Toda capacidad sobre la infraestructura (migraciones, reinicio de servicios, lectura de archivos, git) se expondrá como tools MCP discretas y nombradas, implementadas como código determinista en `mcp_servers/` — la lista de tools disponibles será la política de seguridad del agente autónomo, a diferencia de Claude Code (sesión `iac`, supervisada por Fernando), que sí tiene shell completo bajo `.claude/settings.json` y el hook de ADR-007. Ver ADR-015. Primer paso ya dado (ver ADR-020): `mcp_servers/django_project/` tiene dos tools reales de solo lectura (`git_status`, `read_file`), probadas y sin wiring a ningún orquestador todavía porque no existe. `run_migrations`/`restart_web`/`run_tests` (tools con efectos reales) quedan pendientes a propósito para una entrega posterior.

Para autenticar (ver ADR-016), está decidido que el contenedor del orquestador monte el credencial de suscripción de `~/.claude/.credentials.json` en solo lectura, corriendo con el UID del propietario del archivo en el host — condicionado a que ninguna tool MCP permita lectura de rutas arbitrarias del filesystem, ya que el credencial es entorno de ejecución del proceso, no una capacidad invocable por el agente. Si esa condición deja de cumplirse, la decisión queda invalidada y hay que pasar a clave de API dedicada. Ver ADR-017. **Pendiente (Fase 3):** esto solo se probó en un contenedor de test descartable (ver ADR-017); no hay orquestador real corriendo todavía, así que la condición hoy se cumple de forma vacía, no activa.
