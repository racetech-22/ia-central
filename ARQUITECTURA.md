...
**Pendiente (forma mínima: Fase 3 — sin persistencia; forma final: Fase 5 — persistida en Django/Postgres dentro del panel, ver ADR-013).** Arquitectura de dos roles conversando, con Fernando como decisor final: un rol planificador (conversacional, similar al uso actual de Cowork) y un rol ejecutor, los tres en una misma ventana de chat. Formaliza dentro de IA CENTRAL el patrón que hoy requiere que Fernando relaye manualmente entre Cowork y la sesión `iac` de Claude Code.
...
1. Sala de discusión de a tres, forma mínima (§2) — Fase 3. El rol ejecutor es Claude Code real (sesión `iac`), no el orquestador angosto — falta diseñar el puente entre la sala y ese proceso, ver ADR-024.
