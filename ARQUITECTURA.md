...
Primer pilar en el orden de prioridad de ADR-024 — la pieza con más base ya construida de las cinco (persistencia de conversación de ADR-013 casi gratis, orquestador angosto para lo autónomo), pero no toda: el puente real entre la sala y el proceso de Claude Code (para que el rol ejecutor sea Claude Code de verdad, no una simulación) es una pieza técnica nueva, todavía sin diseñar — ver Consecuencias de ADR-024. Puede arrancar sin esperar al motor de confianza/permisos unificado (segundo pilar en prioridad) porque reusa el modelo de permisos ya existente de Claude Code tal cual.
...
1. Sala de discusión de a tres, forma mínima, **junto con** la memoria básica (historial de conversación en Postgres) (§2) — Fase 3, un solo bloque de trabajo, no en secuencia: una sala sin memoria de conversaciones anteriores reproduce el mismo problema que se busca resolver, solo que dentro de una interfaz. El rol ejecutor es Claude Code real (sesión `iac`), no el orquestador angosto — falta diseñar el puente entre la sala y ese proceso, ver ADR-024.
...
- ADR-025: Diseño técnico inicial de la sala de discusión (pilar 4 de ADR-024) — **en progreso**, no una decisión cerrada. Resuelto: puente de permisos vía `can_use_tool`; roles como agente principal + sub-agente en una sesión del SDK (proyectos nativos); agente propio por destino con conexión saliente (proyectos externos); autenticación por proyecto vía Workspaces de Anthropic (una clave de API por proyecto, aislando cupo y costo); esquema de datos (Proyecto/Chat/tabla espejo de `SessionStore`/SolicitudPermiso) y cifrado de credenciales de proyecto (campo propio sobre `cryptography`). Sin resolver: protocolo exacto de conexión saliente del agente remoto, pantalla del panel para conectar un proyecto, y si el modelo de sub-agente sigue siendo válido una vez que exista el motor de permisos configurable.
