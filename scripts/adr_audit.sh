...
También verificá (f), que docs/estado.yml (fuente estructurada del mapa de ruta, ver ADR-029 — apps/adminpanel/templates/adminpanel/mapa.html solo la renderiza, no la declara) no contradiga a ARQUITECTURA.md ni a las ADR. Cada pieza de la sección "piezas" declara un campo "estado" con uno de tres valores ASCII: construido, disenado, o pendiente. Comprobá que ese valor coincida con lo que dice la prosa de la ADR (o ADR) listada en el campo "adrs" de esa pieza, y con ARQUITECTURA.md: reportá como discrepancia toda pieza que docs/estado.yml declare "construido" cuando la ADR correspondiente la describe como pendiente o no implementada, o al revés, o "disenado" cuando en realidad ya está construida o todavía no tiene ni diseño. No hace falta revisar si las ADR referenciadas existen como archivo ni si los artefactos de una pieza "construido" existen en disco — eso ya lo bloquea el hook de pre-commit de ADR-029 antes de que el commit llegue a existir en la rama, así que si estás leyendo esto en master esa capa ya pasó; esta verificación es sobre el juicio de lectura de la prosa, no sobre existencia de archivos. docs/estado.yml es un resumen estructurado, no una fuente de verdad — ante cualquier diferencia, mandan ARQUITECTURA.md y las ADR, y lo que hay que corregir es docs/estado.yml. Si el archivo no existe, no lo reportes como discrepancia.

No modifiques ni borres nada. Terminá con un resumen en texto plano, en este orden: primero las DISCREPANCIAS reales de contenido (si hay alguna), con el ADR, la afirmación exacta, y qué encontraste en el repo en su lugar. Después, un bloque aparte para las discrepancias de INDEX.md (d1/d2/d3) si las hay, con la misma prioridad que las discrepancias de contenido — un INDEX.md desactualizado también es una fuente de verdad rota. Después, un bloque aparte para las discrepancias de coherencia interna (e1/e2/e3) si las hay, con la misma prioridad que las anteriores — una referencia interna rota o un conteo desactualizado es tan una fuente de verdad rota como las anteriores. Después, un bloque aparte para las discrepancias de coherencia del mapa (f) si las hay, con la misma prioridad que las anteriores — un pilar mostrado como construido cuando no lo está (o al revés) es tan fuente de verdad rota como las anteriores. Después, aparte y con menor énfasis, las AMBIGÜEDADES. Si no hay discrepancias, decilo explícitamente. No detalles los casos CORRECTOS uno por uno, mencionalos de forma agregada (cuántos, cuáles ADR).

Además de ese resumen en texto plano, terminá tu respuesta con una última línea, sola, exactamente 'ADR_CONTENT_STATUS=CLEAN' si no encontraste ninguna DISCREPANCIA de contenido, ni discrepancia de INDEX.md (d1/d2/d3), ni de coherencia interna (e1/e2/e3), ni de coherencia del mapa (f), o exactamente 'ADR_CONTENT_STATUS=DISCREPANCIES_FOUND' si encontraste al menos una de cualquiera de esos cuatro tipos. Las AMBIGÜEDADES por sí solas, sin ninguna DISCREPANCIA real, no cuentan para este marcador — dejalo en CLEAN si sólo hay ambigüedades. Esta línea tiene que aparecer literalmente, sin texto adicional en la misma línea, porque un proceso automático la va a buscar con grep." \
