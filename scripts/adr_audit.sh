...
También verificá (f), que apps/adminpanel/templates/adminpanel/mapa.html (hoja de ruta visual) no contradiga a ARQUITECTURA.md ni a las ADR. El mapa marca cada pilar y cada pieza de la base construida con uno de tres estados: construido, diseñado sin código, o pendiente. Comprobá que ese estado coincida con lo que dicen ARQUITECTURA.md y la ADR correspondiente: reportá como discrepancia todo pilar o pieza que el mapa muestre como construido cuando la ADR lo marca como pendiente o no implementado, o al revés. El mapa es un resumen visual, no una fuente de verdad — ante cualquier diferencia, mandan ARQUITECTURA.md y las ADR, y lo que hay que corregir es el mapa. Si el archivo no existe, no lo reportes como discrepancia.
