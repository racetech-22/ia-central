...
- `apps/adminpanel/templates/adminpanel/mapa.html` (hoja de ruta visual, accesible en `/mapa/` para superusuarios, enlazada desde la portada del admin) se actualiza en el mismo commit que cualquier cambio que altere el estado de un pilar o de la base construida — igual que ARQUITECTURA.md y CHANGELOG.md. Es un resumen visual, no una fuente de verdad: si contradice a ARQUITECTURA.md o a una ADR, manda la ADR y hay que corregir el mapa.
...
**Asimetría verificada el 2026-08-06: el protocolo de dos pasos no es ejecutable desde Cowork.** El paso 1 (pedir el SHA a `https://api.github.com/repos/racetech-22/ia-central/commits/master`) funciona desde Claude Code y desde una terminal del VPS, pero devuelve vacío desde la herramienta de fetch web de Cowork — mismo síntoma que ADR-011 ya había registrado para el endpoint `contents/`. Consecuencia práctica: una sesión de Cowork no puede anclar a SHA y queda expuesta al CDN de GitHub, que ese día sirvió versiones desactualizadas de ARQUITECTURA.md, ADR-024 y ADR-027 (sin sus enmiendas del mismo día) en un fetch directo a `raw.githubusercontent.com` sobre el nombre de rama. Mitigación disponible hoy en Cowork, imperfecta: reintentar cada archivo con un query string distinto en la misma sesión y comparar — si la segunda lectura trae contenido que la primera no tenía, la primera estaba cacheada. No es una garantía (el CDN puede ignorar el query string para su cache key, que es justamente por lo que se abandonó el `?v=`), pero es lo único que funcionó. **Regla operativa para Cowork: antes de razonar sobre una ADR con estado "En progreso" o con enmiendas recientes, releerla una segunda vez con query string distinto y confirmar que trae lo mismo.**
